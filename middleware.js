...
module.exports.isLoggedInReview = (req, res, next) => {
  if (!req.isAuthenticated()) {
    const id = req.params.id || req.originalUrl.split("/")[2];
    req.session.redirectUrl = `/listings/${id}`; // redirect to listing page
    req.flash("error", "Please log in to post a review.");
    return res.redirect("/login");
  }
  next();
};
